...
def save_config(config, output_path="config.yaml"):
    ...
    print(f"[OK] 配置文件已保存: {output_path}")
...
jobs:
  build-deploy:
    ...
    steps:
      ...
      - name: Install PDF toolchain and fonts
        env:
      ...
      - name: Deploy to GitHub Pages
        ...
        with:
          ...
          publish_dir: source/source_build/html
          force_orphan: true
...
"""
    ...
    print(f"[OK] GitHub Actions工作流已创建: {workflow_path}")
...
"""
    ...
    print(f"[OK] Read the Docs配置文件已创建: {rtd_path}")
...
"""
    ...
    print(f"[OK] 版本列表文件已创建: {versions_path}")
...
def main():
    ...
    if not Path("doc_generator.py").exists():
        print("[ERROR] 错误: 请在source目录中运行此脚本")
        sys.exit(1)
